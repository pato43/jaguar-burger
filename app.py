...
<style>
  ...
  h1 {font-size: 2.35rem !important;} h2 {font-size: 1.7rem !important;} h3 {font-size: 1.28rem !important;}
...
def gexplain(text): st.write_stream(stream_text(f"🧠 Explicación (Gemini): {text}"))
...
# ─────────────────────────── Encabezado + chips de estado
st.markdown("# Jaguar Burger MX — Plataforma de Ventas, Rentabilidad y Modelos")
s = st.columns(5)
with s[0]: st.markdown('<span class="chip"><span class="dot"></span> Conectado a Snowflake</span>', unsafe_allow_html=True)
with s[1]: st.markdown('<span class="chip"><span class="dot"></span> Gemini ADK activo</span>', unsafe_allow_html=True)
with s[2]: st.markdown('<span class="chip"><span class="dot"></span> PyDeck OK</span>', unsafe_allow_html=True)
with s[3]: st.markdown('<span class="chip"><span class="dot"></span> Plotly listo</span>', unsafe_allow_html=True)
with s[4]: st.markdown('<span class="chip"><span class="dot"></span> Streamlit Live</span>', unsafe_allow_html=True)

st.markdown("""
<div class="hero">
<b>Propósito:</b> visibilizar desempeño por sucursal, explicar resultados con apoyo analítico y detectar oportunidades en tiempo real.
<br/><b>Módulos:</b> KPIs ejecutivos, exploración por tiempo/tienda/estado, panel de modelos, clustering operativo y mapa con calor de ventas.
</div>
""", unsafe_allow_html=True)
...
with tab_kpi:
    ...
    gexplain("KPIs con tendencia de 90 días; cambios de margen suelen asociarse a promociones y mix de ticket.")
    ...
    with a:
        ...
        gexplain("Evolución mensual: identifica estacionalidad y periodos de mayor demanda.")
    with b:
        ...
        gexplain("Participación por estado para priorizar inversión y cobertura.")

    c, d = st.columns([1.35,1])
    with c:
        ...
        gexplain("Mapa de calor: ubica picos operativos por día y mes.")
    with d:
        ...
        gexplain("Top sucursales por ventas con ticket y margen para benchmarking.")

# ─────────────────────────── Exploración
with tab_explore:
    subtab = st.tabs(["Por Mes", "Por Tienda", "Por Estado", "Distribuciones", "Descomposición", "Pareto 80/20"])
    with subtab[0]:
        ...
        with g1:
            ...
            st.plotly_chart(fig, use_container_width=True)
            gexplain("Ingresos mensuales con área para remarcar magnitud y tendencia.")
        with g2:
            st.dataframe(dfm, use_container_width=True, hide_index=True)
            gexplain("Detalle por mes con pedidos, marketing, utilidad, margen y ticket.")
    with subtab[1]:
        ...
        with t1:
            ...
            st.plotly_chart(fig, use_container_width=True)
            gexplain("Ranking de sucursales y focos de crecimiento.")
        with t2:
            st.dataframe(dft.sort_values("ventas", ascending=False), use_container_width=True, hide_index=True)
            gexplain("Tabla completa para filtros y exportación.")
    with subtab[2]:
        ...
        with s1:
            ...
            st.plotly_chart(fig, use_container_width=True)
            gexplain("Panorama por estado para priorizar inversiones.")
        with s2:
            st.dataframe(dfs.sort_values("ventas", ascending=False), use_container_width=True, hide_index=True)
            gexplain("Cifras agregadas por estado.")
    with subtab[3]:
        dx1, dx2 = st.columns([1.35,1])
        with dx1:
            fig = px.histogram(F, x="ticket_avg_mxn", nbins=30, marginal="box")
            st.plotly_chart(fig, use_container_width=True)
            gexplain("Distribución de ticket: colas y outliers.")
        with dx2:
            fig = px.scatter(F, x="marketing_mxn", y="sales_mxn")
            st.plotly_chart(fig, use_container_width=True)
            gexplain("Relación marketing–ventas y elasticidad aparente.")
    with subtab[4]:
        ...
        gexplain("Puente Ventas → Utilidad para entender impactos.")
    with subtab[5]:
        ...
        fig = go.Figure()
        fig.add_trace(go.Bar(x=pareto["store_name"], y=pareto["sales_mxn"], name="Ventas"))
        ...
        gexplain("Análisis de Pareto 80/20: pocas tiendas concentran la mayor parte de las ventas.")
...
with tab_models:
    ...
    with colA:
        ...
        st.plotly_chart(fig, use_container_width=True)
        gexplain("La cercanía a la línea 45° indica buena calibración del modelo de regresión.")
        tbl = pd.DataFrame({"Muestra": np.arange(1, 21), "Real": real[:20].round(0).astype(int), "Predicho": pred[:20].round(0).astype(int)})
        st.dataframe(tbl, use_container_width=True, hide_index=True)
        gexplain("Muestra de predicciones para inspección puntual.")
    with colB:
        ...
        gexplain("Matriz de confusión para entender aciertos y errores del clasificador.")
        ...
        gexplain("Capacidad discriminativa a distintos umbrales.")

# ─────────────────────────── Clustering (mejorado)
with tab_cluster:
    agg = (F.groupby(["store_id","store_name","estado","ciudad","lat","lon"])
             .agg(ventas=("sales_mxn","sum"), margen=("margin_pct","mean"), ticket=("ticket_avg_mxn","mean"),
                  pedidos=("orders","sum")).reset_index())
    if agg.empty:
        st.info("No hay datos con los filtros actuales.")
    else:
        # cluster sintético más vistoso
        sv, sm = (agg["ventas"]-agg["ventas"].min())/(agg["ventas"].max()-agg["ventas"].min()+1e-9), \
                 (agg["margen"]-agg["margen"].min())/(agg["margen"].max()-agg["margen"].min()+1e-9)
        agg["pc1"] = sv*1.2 + default_rng().normal(0, .05, len(agg))
        agg["pc2"] = sm*1.1 + default_rng().normal(0, .05, len(agg))
        agg["cluster"] = pd.qcut(agg["ventas"] + agg["margen"]*agg["ventas"].median(), 3, labels=[0,1,2]).astype(int)
        ...
        with c1:
            ...
            st.plotly_chart(fig, use_container_width=True)
            gexplain("Clusters por volumen y rentabilidad; tamaño del punto ~ pedidos.")
        with c2:
            ...
            gexplain("Perfiles por cluster para estrategias diferenciadas.")
...
with tab_map:
    ...
    if stores_latest.empty:
        ...
        stores_latest = stores_latest.dropna(subset=["lat","lon"]).astype({"lat":"float64","lon":"float64"})
        ...
        view_state = pdk.ViewState(latitude=23.6, longitude=-102.5, zoom=4.6, pitch=35, bearing=0)
        deck = pdk.Deck(layers=[hex_layer, scatter, text],
                        initial_view_state=view_state,
                        tooltip={"text":"{store_name}\n{estado} — {ciudad}\nVentas: {ventas_label}"})
        st.pydeck_chart(deck, use_container_width=True)
        gexplain("Puntos y hexágonos 3D escalan con ventas; el mapa muestra concentración geográfica por sucursal.")
        ...
        with m1:
            ...
            gexplain("Top sucursales por ventas con margen y ticket.")
        with m2:
            ...
            fig.update_layout(xaxis_title="Estado", yaxis_title="Ventas (MXN)")
            st.plotly_chart(fig, use_container_width=True)
            gexplain("Contribución por estado basada en agregados geoespaciales.")
